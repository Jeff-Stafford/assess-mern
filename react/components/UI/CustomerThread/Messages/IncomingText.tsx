...
interface IncomingTextProps {
	note: CustomerThreadNote;
	createdBy: string;
	position: 'left' | 'right';
}

const IncomingText: FC<IncomingTextProps> = ({ children, note, createdBy }) => {
	return (
		<div className="bg-note-incoming-text w-full max-w-360 p-2.5 rounded-xl">
			<p className="text-note-text-green text-xs mb-0.5">
				Text Message to <span className="font-semibold">{createdBy}</span>
			</p>
			<p>{children}</p>
			{note.note_url && (
				<a
